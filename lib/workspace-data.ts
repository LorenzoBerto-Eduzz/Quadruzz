...
export async function getWorkspacePayload(user: ChatGPTUser): Promise<WorkspacePayload> {
  ...
  if (!member || member.status !== 'approved') {
    const request = await db.prepare("SELECT status,display_name,profile_image_key,expires_at FROM access_requests WHERE user_id=? AND status='pending' AND expires_at>?").bind(user.userId, now).first<RequestRow>();
  ...
  const requests = (await db.prepare("SELECT user_id AS userId,email,requested_at AS requestedAt FROM access_requests WHERE status='pending' AND expires_at>? ORDER BY requested_at ASC").bind(now).all<PendingRequest>()).results;
