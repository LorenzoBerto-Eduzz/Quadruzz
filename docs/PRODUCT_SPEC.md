...
- The current unpacked build is in `project/extension/`. An extension-owned offscreen document and Web Worker keep authenticated synchronization and presence active regardless of the focused page. Ordinary HTTP(S) pages retain the persistent floating top-right overlay, including packaged content-script recovery when permitted; Chrome-protected pages use Chrome's native extension action popup, which dismisses on outside focus. The toolbar action and Alt+Shift+Q toggle the member popup, Alt+Shift+D opens acting-status controls, and Alt+Shift+S opens note controls. Chrome 127 or newer is required.
...
- Approved members can set or clear one server-authoritative note of up to two visual lines. Alt+Shift+S opens the note editor directly. The offscreen worker is the single note-change detector. New or replaced notes produce a five-second newest-first overlay stack on an injectable page. On a protected page they open the native action popup with the notification stack for five seconds and then close it; a native Chrome notification is used only if Chrome refuses to open the popup. If the member popup is already open, its note updates in place and performs one 1.5-second dark-gray-to-white freshness blink, consuming the notification so it is not replayed after closing. Opening Q/D/S dismisses any protected-page notification stack and shows the requested member, acting-status, or note view. Notifications exclude sessions belonging to the author and never replay historical notes or removals.
...
- Extension activity expires after 30 seconds without authenticated Companion traffic; authenticated member-list reads renew the fallback reader session at least every 10 seconds when needed.
