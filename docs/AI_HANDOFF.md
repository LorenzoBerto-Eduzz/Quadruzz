...
- The Companion now keeps authorization, presence, access, and note synchronization in an extension-owned offscreen worker, independent of the focused page. Ordinary HTTP(S) pages retain the persistent floating top-right overlay; Chrome-protected pages use Chrome's native extension action popup, which closes when focus moves outside it. The toolbar icon and Alt+Shift+Q toggle the member popup, Alt+Shift+D toggles acting-status controls, and Alt+Shift+S toggles note controls. A compact house button at the left of the member-overlay header opens the Cross HQ in a new tab; close remains on the right. Hidden mode stays active for future notifications. Chrome 127 or newer is required for programmatic action-popup opening from the popup/role/note shortcuts.
- Companion remains version `0.1.0` while its first release is still being developed. The canonical unpacked and distributable source is `project/extension/`; zipping that folder's contents with `manifest.json` at the archive root produces the same extension package used for colleague testing and Chrome Web Store upload.
