...
- Companion development version is `0.1.2`. The unpacked source is `project/extension/`; the current colleague/Store-ready archive is `local_assets/cross-quadruzz-extension-0.1.2.zip`. The earlier `0.1.0` Chrome Web Store draft was submitted for review, while local testing continues on `0.1.2`.
